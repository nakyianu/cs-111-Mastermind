...
class Column():
    ...
    def columnPlay(self, mainList, count, window):          #draws the four balls for a guess
        '''Draws the objects in the columns'''
...
class listrow():        
    '''#makes a class for each play which contains the guess and the feedback.'''

    def __init__(self, guess, feedback):
        self.guess = guess
        self.feedback = feedback

    def getFeedback(self):                      #returns the feedback
        return self.feedback

    def getPlay(self):                          #returns the guess
        return self.guess
...
def possibilities(guess, length):  
    '''Creates a list of all the possible iterations 
    once the computer guesses the four correct colors.'''    
    ...
    if length == 1:                             #base case
        ...
        result = []
        for first in guess:
            for rest in possibilities(guess, length-1):         #recursively finds all the permutations of the guessed colors               
                result.append(first + ", " + rest)              #creates a list of all the options
    
    for u in guess:                                             #runs through the options and makes sure there are no repeated colors.
        i = 0
        while i < len(result): 
...
def goodPossibilities(result, listguess, count):
    '''Finds the most plausible possiblites
    It gets rid of options which have colors in the same places as previous guess where the feedback was all white.''' 

    for x in range(count):                                      #it will go through each of the previous plays
        if "black" not in listguess[x].getFeedback():           #finds guesses which had only white feedback
            play = listguess[x].getPlay()                       
            for y in play:                                      #goes through each color in a guess
                i = 0
                while i < len(result):                          #runs through the list; removes options with colors in the same positions.
                    index = play.index(y)
                    z = result[i]                               #result is the list of all the possibilities
                    aa = z.split(", ")                      
                    if y == aa[index]:                          #compares each color in each position and if there is an option that has the same color in the same position it removes them.
                        result.remove(z)
                    else:
                        i += 1
    return result
...
def repeat(guess, listguess, count):
    ...
    for i in range(count):
        if guess == listguess[i].getPlay():                         #if the guess matches a previous guess it returns true, otherwise it returns false
            return True
    return False

def noRepeat(guess, listguess, count):
    '''shuffles a guess if it has been played'''                    #if repeat returns false then the guess does not needd to be altered.

    if repeat(guess, listguess, count) == False:
        return guess
    else:
        random.shuffle(guess)                                       #if it returns true, the guess is shuffled.
        random.shuffle(guess)
        noRepeat(guess, listguess, count)
    return guess 


def replay(colorlist, window):  
    '''Allows user to replay'''   

    playagain = input("Do you want to play again? Y or N: ")
    if playagain == "Y":                                            #if the player inputs Y the game restarts
        window.close()                                              #closes the window so they don't stack up
        gameplay(colorlist)
    else:
        return print("Thanks for playing! See you soon.")           #If the player inputs anything other than Y, the game closes.



listguess = ["guess1", "guess2", "guess3", "guess4", "guess5", "guess6", "guess7", "guess8", "guess9", "guess10"]
#When a row object is created, it takes the name of the next index in the list, 
    #and replaces the string at that index with the new object'''
    ...
    if feedbackCompare == [""]:                    #allows the user to exit by pressing enter.
        ...
        if len(feedbackCompare) == 1:
            ...
            if feedbackCompare.count('white') == 3:            #if it gets three whites then it shuffles the whole list. 
                ...
                guessCompare[2], guessCompare[3] = guessCompare[3], guessCompare[2]         #if it has three blacks, it just swaps the last colour out and does nothing.                                                    
            ...
            list1 = []                          #creates two empty lists to turn goodResults from a list of strings into a list of lists.
            list2 = []
            for item in strgoodResults:         #turns goodResults into a list of lists
                list1 = item.split(", ")
                list2.append(list1)
            goodResults = list2   
            listguessCompare = random.sample(goodResults, 1)        #randomly selects an option to play.    
            guessCompare = listguessCompare[0]                      #retrieves the first item from listguessCompare which is a list containing one list    
...
def gameplay(colorlist):   
    ...
    if human == "mastermind":               #if the human is mastermind this is how the game proceeds.
        ...
        for q in range(4):                      #turns each ball into an object
            masterlist[q] = Mainball(masterlist[q], q + 1)
